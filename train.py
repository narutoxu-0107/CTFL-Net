...
from timm.loss import JsdCrossEntropy, SoftTargetCrossEntropy, BinaryCrossEntropy, LabelSmoothingCrossEntropy, MSELoss, SmoothL1Loss, HuberLoss
from timm.loss.custom_loss import L1Loss
...
parser.add_argument('--data-val-dir-0',default=None, metavar='DIR',
                    help='path to dataset (root dir)')
parser.add_argument('--data-val-dir-1',default=None, metavar='DIR',
                    help='path to dataset (root dir)')
parser.add_argument('--data-val-dir-2',default=None, metavar='DIR',
                    help='path to dataset (root dir)')
...
"""添加用于回归训练的标记， 默认为回归训练"""
group.add_argument('--is-regression', type=bool, default=True,
...
group.add_argument('--modification',default='none', type=str,help='description on the modification')
group.add_argument('--loss-fn',default='mse', type=str,help='type of loss_funtion')
...
def _parse_args():
    ...
    if args_config.config:
        with open(args_config.config, 'r') as f:
            cfg = yaml.safe_load(f)  # 使用yaml安全加载配置文件内容
...
def main():
    ...
    if args.amp:
        if args.amp_impl == 'apex':
            ...
            use_amp = 'native'  # 使用PyTorch原生的AMP
    ...
    if args.val_split:
        dataset_eval_0 = create_dataset(
            args.dataset,
            root=args.data_val_dir_0,
        ...
        dataset_eval_1 = create_dataset(
            args.dataset,
            root=args.data_val_dir_1,
            split=args.val_split,
            is_training=False,
            class_map=args.class_map,
            download=args.dataset_download,
        ...
        dataset_eval_2 = create_dataset(
            args.dataset,
            root=args.data_val_dir_2,
            split=args.val_split,
            is_training=False,
            class_map=args.class_map,
            download=args.dataset_download,
    ...
    if args.val_split:
        ...
        loader_eval_0 = create_loader(
            ...
            device=device,
            use_prefetcher=args.prefetcher,
            # crop_img_size = args.crop_img_size #中心裁剪图像大小
        )
        loader_eval_1 = create_loader(
            dataset_eval_1,
            input_size=data_config['input_size'],
            batch_size=args.validation_batch_size or args.batch_size,
            is_training=False,
    ...
    # setup loss function
    """添加MSELoss"""
    if args.is_regression == True:
        if args.loss_fn == 'mse':
            train_loss_fn = MSELoss()
        elif args.loss_fn == 'SmoothL1Loss':
            train_loss_fn = SmoothL1Loss()
        elif args.loss_fn == 'HuberLoss':
            train_loss_fn = HuberLoss()
        elif args.loss_fn == 'L1Loss':
            train_loss_fn = L1Loss()
    elif args.jsd_loss:
    ...
    eval_metric = args.eval_metric if loader_eval_0 is not None else 'loss'
    ...
    try:
        ...
        val_time= time.time()
        validate(model,loader_eval_0,
            validate_loss_fn, args,
            device=device, amp_autocast=amp_autocast,
        )
        validate(model, loader_eval_1,
                 validate_loss_fn, args,
                 device=device, amp_autocast=amp_autocast,
                 )
        validate(model, loader_eval_2,
                 validate_loss_fn, args,
                 device=device, amp_autocast=amp_autocast,
                 )
        if args.model_ema:
            val_time = (time.time() - val_time)*2
        else:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if loader_eval_0 is not None:
                eval_metrics_0 = validate(
                    model,
                    loader_eval_0,
                    validate_loss_fn,
                    args,
            ...
            if output_dir is not None:
                ...
                utils.update_summary(
                    ...
                    train_metrics,
                    eval_metrics_0,
                    filename=os.path.join(output_dir, 'summary_0.csv'),
                    lr=sum(lrs) / len(lrs),
                    # write_header=best_metric is None,  # 如果是第一个epoch，则写入标题头
                ...
                utils.update_summary(
                    ...
                    train_metrics,
                    eval_metrics_1,
                    filename=os.path.join(output_dir, 'summary_1.csv'),
                    lr=sum(lrs) / len(lrs),
                    # write_header=best_metric is None,  # 如果是第一个epoch，则写入标题头
                    write_header=True if epoch == 0 else False,
                    log_wandb=args.log_wandb and has_wandb,
                ...
                utils.update_summary(
                    ...
                    train_metrics,
                    eval_metrics_2,
                    filename=os.path.join(output_dir, 'summary_2.csv'),
                    lr=sum(lrs) / len(lrs),
                    # write_header=best_metric is None,  # 如果是第一个epoch，则写入标题头
                    write_header=True if epoch == 0 else False,
                    log_wandb=args.log_wandb and has_wandb,
            ...
            if eval_metrics_0 is not None:
                latest_metric = eval_metrics_0[eval_metric]
            else:
            ...
            results_0.append({
                'epoch': epoch,
                'train': train_metrics,
                'validation': eval_metrics_0,
            })
            results_1.append({
                'epoch': epoch,
                'train': train_metrics,
                'validation': eval_metrics_1,
            })
            results_2.append({
                'epoch': epoch,
                'train': train_metrics,
                'validation': eval_metrics_2,
            })
            if (saver is not None) and epoch>0 :
                if eval_metrics_0.get('r2')>max(r2_lst_0):
                    max_0=epoch
                if eval_metrics_1.get('r2') > max(r2_lst_1):
                    max_1 = epoch
                if eval_metrics_2.get('r2')>max(r2_lst_2):
                    max_2=epoch
                print(f'{results_0[max_0]}')
                print(f'{results_1[max_1]}')
                print(f'{results_2[max_2]}')
            r2_lst_0.append(eval_metrics_0.get("r2"))
            r2_lst_1.append(eval_metrics_1.get("r2"))
            r2_lst_2.append(eval_metrics_2.get("r2"))
    except KeyboardInterrupt:
        pass

    results = {'all': results_0}

    # 打印结果，使用json格式美化输出
    print(f'--result\n{json.dumps(results_0, indent=4)}')
    print(f'--result\n{json.dumps(results_1, indent=4)}')
    print(f'--result\n{json.dumps(results_2, indent=4)}')
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        data_load_lst.append(data_load_end-data_load_start)
        ...
        all_time = update_time_m.avg*len(loader)*num_epochs+val_time*num_epochs
        current_time = update_time_m.avg*len(loader)*epoch+val_time*epoch+ update_time_m.sum
        all_time = str(timedelta(seconds=int(all_time)))
            ...
            if utils.is_primary(args):
                if args.is_regression:
                    _logger.info(
                        ...
                        f'Mse: {batch_mse:#.3g} ({sum_mse:#.3g}) '
                        f'R2: {batch_r2:#.3g} ({sum_r2:#.3g}) '
                        f'Time: {update_time_m.val:.3f}s, {update_sample_count / update_time_m.val:>7.2f}/s '
                        f'({update_time_m.avg:.3f}s, {update_sample_count / update_time_m.avg:>7.2f}/s) '
                        f'LR: {lr:.3e}  '
                        f'Data: {data_time_m.val:.3f} ({data_time_m.avg:.3f}) '
                        f'Time remainin: {current_time}/{all_time}  '
                    )
                else:
                    _logger.info(
                        f'Train: {epoch} [{update_idx:>4d}/{updates_per_epoch} '
    ...
    print(f'data_load_time: {round(sum(data_load_lst)/len(data_load_lst),5)};\nforward_time: {round(sum(forward_lst)/len(forward_lst),5)};\nbackward_time: {round(sum(backward_lst)/len(backward_lst),5)}')
            ...
            if utils.is_primary(args) and (last_batch or batch_idx % args.log_interval == 0):
                ...
                if not args.is_regression:
                    ...
                    _logger.info(
                        ...
                        f'Time: {batch_time_m.val:.3f} ({batch_time_m.avg:.3f}) '
                        f'Loss: {losses_m.val:>7.3f} ({losses_m.avg:>6.3f}) '
                        f'Mse: {batch_mse:>7.3f} ({sum_mse:>6.3f}) '
                        ...
                        f'Nrmse: {batch_nrmse:>6.3f} ({sum_nrmse:>6.3f}) '
                    )

    if not args.is_regression:
        metrics = OrderedDict([('loss', losses_m.avg), ('top1', top1_m.avg), ('top5', top5_m.avg)])
    else:
        metrics = OrderedDict([('loss', float(sum_mse)), ('rmse', float(sum_rmse)), ('r2', float(sum_r2)), ('rrmse', float(sum_rrmse)), ('mae', float(sum_mae)),('nrmse', float(sum_nrmse))])
    return metrics
