...
def extract_tarinfos(
    ...
    if not is_regression:
        ...
        samples_and_targets = [(s, float(l.split('/')[-1])) for s, l in zip(samples, labels) if l in class_name_to_idx]
    if sort:
...
class ReaderImageInTar(Reader):
    ...
    def __init__(self, root, class_map='', cache_tarfiles=True, cache_tarinfo=None, is_regression=False):
        super().__init__()
