...
class HATModule(nn.Module):
    def __init__(self, in_channels, reduction=16):  #reduction=16，可以改为8，4
        super(HATModule, self).__init__()
        # 通道注意力机制（类似于SE模块）
        ...
        # 空间注意力机制
        self.spatial_conv = nn.Conv2d(2, 1, kernel_size=7, padding=3, bias=False)  #kernel_size=7，可以改成5，3

    def forward(self, x):
        # 通道注意力
        avg = self.avg_pool(x)
        channel_att = self.fc(avg) * x

        # 空间注意力
        max_pool, avg_pool = torch.max(channel_att, dim=1, keepdim=True)[0], torch.mean(channel_att, dim=1,
                                                                                        keepdim=True)
...
class MetaNeXtStage(nn.Module):
    def __init__(
            ...
            norm_layer=None,
            mlp_ratio=4,
            use_hat=False,  # 是否使用 HAT 模块
            hat_module=None,  # HAT 模块
        ...
        if self.use_hat:
            assert hat_module is not None, "When use_hat is True, hat_module must be provided"
            self.hat = hat_module(out_chs)  # 初始化 HAT 模块
...
class MetaNeXt(nn.Module):
    ...
    def __init__(
            ...
            drop_path_rate=0.,
            ls_init_value=1e-6,
            use_hat=False,  # 控制是否在各阶段插入 HAT 模块
            hat_module=HATModule,  # 传入 HAT 模块
    ):
        super().__init__()
        ...
        for i in range(num_stage):
            ...
            self.stages.append(MetaNeXtStage(
                ...
                use_hat=use_hat,  # 控制是否在此阶段使用 HAT 模块
                hat_module=hat_module,  # HAT 模块
            ))
            prev_chs = out_chs
...
@register_model
def hat_inception_next(pretrained=False, **kwargs):
    model_args = dict(
        depths=(3, 3, 27, 3),  # 控制每个阶段的深度  #原来使用的（3，3，27，3）
        dims=(96, 192, 384, 768),  # 控制每个阶段的维度
        token_mixers=InceptionDWConv2d,  # 使用 Inception NeXt 中的 token mixer
        use_hat=True,  # 启用 HAT 模块
        hat_module=HATModule,  # 传入 HAT 模块
    )
    return _create_inception_next('hat_inception_next', pretrained=pretrained, **dict(model_args, **kwargs))